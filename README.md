...
## Ответы на опросы:
Определите разницу между контейнером и образом - образ это можно сказать дистрибутив программы, лежит на диске но толку мало. Контейнер это уже запущеный экземпляр той самый программы. Рабоате и выполняет заложенные в нее функции.  

Можно ли в контейнере собрать ядро? - нет, контейнер использует ядро системы в которой он запущен,  у него нет своего ядра. Хотя, вот если пофантазировать, навреное можно внутри контейнера собраь ядро, но в данном случае контейнер будет рпосто инстрментом сборки, затем мы возьмем это ядро и применим уже на какой либо корневой машине. То есть всетаки ответбудет -  да, можно, но только собрать, не запустить.
